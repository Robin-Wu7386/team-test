<template>
  <div class="admin-container">
    ...
    <main class="main-content">
      ...
      <!-- 5. 评论管理 (添加了情感分析功能) -->
      <div v-show="activeMenu === 'comment'" class="content-module">
        <div class="module-header">
          <h2>评论列表</h2>
    ...
    <!-- 评论模态框 -->
    <el-dialog v-model="commentModalVisible" :title="'新增评论'">
      <el-form :model="commentForm" label-width="80px" class="modal-form">
        <el-form-item label="内容" required>
          <el-input v-model="commentForm.content" type="textarea" :autosize="{ minRows: 3, maxRows: 6 }"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="commentModalVisible = false">取消</el-button>
        <el-button type="primary" @click="submitCommentForm">新增</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const fetchComments = async () => {
  ...
  try {
    const res = await axios.get(`${API_PREFIX}/comments`, { headers: HEADERS })
    if (res.data.success) {
      commentList.value = res.data.data
    } else {
      ElMessage.error('获取评论列表失败：' + res.data.msg)
    }
  } catch (err) {
    console.error('获取评论列表失败：', err)
    ElMessage.error('获取评论列表失败：' + (err.response?.data?.msg || err.message))
  } finally {
...
const submitCommentForm = async () => {
  ...
  try {
    const res = await axios.post(`${API_PREFIX}/comments`, { content: commentForm.value.content }, { headers: HEADERS })
    if (res.data.success) {
...
const handleCommentDelete = async (id) => {
  try {
    ...
    const res = await axios.delete(`${API_PREFIX}/comments/${id}`, { headers: HEADERS })
    if (res.data.success) {
